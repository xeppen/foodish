...
must_haves:
  truths:
    ...
    - "New rating and complexity fields are nullable for backward compatibility"
  artifacts:
...
Output: Updated schema.prisma with enum types, nullable fields on Meal model, and composite indexes for filtered queries.
</objective>
...
Then extend the Meal model to include these fields as NULLABLE (no @default):
...
CRITICAL:
- Keep fields nullable (Rating?, Complexity?) with NO @default values
- This avoids PostgreSQL P3018 error (enum values must be committed before use as defaults)
- Application code will handle null as NEUTRAL/MEDIUM respectively
- Add usageHistory relation (forward reference to UsageHistory model, will be created in Plan 02)
...
Why nullable: Production app is live. Nullable fields allow old code to run during deployment window. Existing meals will have null for these fields after migration.
  </action>
  <verify>
...
- rating and complexity fields in Meal model (both nullable)
- Three @@index directives in Meal model (userId, userId+rating, userId+complexity)
  </verify>
  <done>
Schema file updated with Rating/Complexity enums, nullable fields added to Meal model, composite indexes defined. Schema passes Prisma validation.
  </done>
</task>
...
- Meal model has rating field (Rating?, nullable)
- Meal model has complexity field (Complexity?, nullable)
