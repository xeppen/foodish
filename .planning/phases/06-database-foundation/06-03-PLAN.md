...
must_haves:
  truths:
    ...
    - "Meals resolve explicit defaults for rating and complexity"
...
Purpose: Execute the migration to add enums, fields, and UsageHistory table safely across environments. Verify backward compatibility so existing code continues working during deployment window.
...
2. ALTER TABLE Meal statements add rating/complexity columns with defaults
...
- Ensure defaults are present for rating/complexity columns
...
- Contains ALTER TABLE "Meal" ADD COLUMN "rating" "Rating" DEFAULT 'NEUTRAL'
- Contains CREATE TABLE "UsageHistory" with ON DELETE CASCADE
- Contains CREATE INDEX statements for composite indexes
  </verify>
  <done>
Migration file created and reviewed. SQL verified with explicit defaults and cascade delete.
  </done>
</task>

<task type="auto">
  <name>Apply migration and generate client (environment-safe)</name>
  <files>prisma/migrations/, prisma/schema.prisma, lib/actions/</files>
  <action>
Apply the migration using environment-safe commands:

```bash
# Local/dev
npx prisma migrate dev

# Production/staging deploy pipeline
npx prisma migrate deploy
npx prisma generate
```

This will:
1. Apply the migration in local development with migration history checks (`migrate dev`)
2. Apply committed migration files in production safely (`migrate deploy`)
3. Regenerate Prisma Client types for the runtime environment
...
IMPORTANT: If migration fails with enum-creation ordering issues, regenerate migration from schema and re-review SQL before applying.
  </action>
  <verify>
Run verification commands:
1. `npx prisma db execute --stdin <<< "SELECT column_name, data_type, is_nullable, column_default FROM information_schema.columns WHERE table_name = 'Meal' AND column_name IN ('rating', 'complexity');"` - should show defaults
2. `npx prisma validate` - should succeed and confirm schema/client compatibility
3. Verify reads and writes succeed without null fallback branches for rating/complexity
  </verify>
  <done>
Migration applied successfully. Database schema updated with enums and UsageHistory table. Prisma Client regenerated with new TypeScript types. Existing and new meals resolve explicit defaults.
  </done>
</task>
...
3. Prisma schema/client validate: `npx prisma validate` (should succeed)
...
- Existing meals display correctly with defaulted ratings/complexity
</success_criteria>
