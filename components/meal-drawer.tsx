  ...
  async function handleAddStarterMeal(starterMeal: MealDrawerProps["starterMeals"][number]) {
    ...
    try {
      ...
      if (!result.alreadyAdded) {
        setStarterMessage(`"${starterMeal.name}" lades till.`);
      }
      const mealId = result.mealId;
      if (!mealId) {
        setStarterMessage("Måltiden lades till men kunde inte öppnas för granskning.");
        router.refresh();
        return;
      }
      setEditorReviewHint("Granska den importerade måltiden innan du sparar. Bild och ingrediensförslag genereras åt dig.");
